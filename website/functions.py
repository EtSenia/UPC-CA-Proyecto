...
def crear_imagen(nodos, rutas):
    G = nx.Graph()

    for id, nodo in nodos.items():
        if nodo.conexiones:
            x, y = latlon_to_xy(nodo.latitud, nodo.longitud)
            if(id == nodo.id):
                G.add_node(str(id), pos=(x, y))

    for v1, v2, v3 in rutas:
    ...
    nx.draw(G, pos, node_size = 1, node_color = 'red', edge_color ='#39FF14', connectionstyle='arc3,rad=0.2', arrows=True, width=0.4)

    plt.savefig('website/static/grafo.png', dpi=300)
...
def dijkstra(grafo, inicio, nodos_a_evitar=[]):
    ...
    fin = time.time()
    print(f"Tiempo: {fin - inicio_t} segundos")
